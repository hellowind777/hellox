...
use super::{normalize_path, ok_tag, warn_tag};

pub(super) fn agent_backend_doctor_lines() -> Vec<String> {
    ...
    match env::var("HELLOX_AGENT_BACKEND_COMMAND") {
        Ok(raw) if !raw.trim().is_empty() => match parse_json_array_env(&raw) {
            ...
            Err(error) => lines.push(format!(
                "{} agent_backend.command: env HELLOX_AGENT_BACKEND_COMMAND invalid: {error}",
                warn_tag()
            )),
        },
        _ => match env::current_exe() {
            ...
            Err(error) => lines.push(format!(
                "{} agent_backend.command: default current_exe unavailable ({error})",
                warn_tag()
            )),
        },
    }
    ...
    inspect_command_prefix_status(&mut lines, &pane_preflight.tmux_command);
    inspect_command_prefix_status(&mut lines, &pane_preflight.iterm_command);
    ...
    if let Some(raw) = pane_preflight.requested_backend_raw.as_deref() {
        if pane_preflight.requested_backend.is_some() {
            ...
            lines.push(format!(
                "{} agent_pane_backend.env: HELLOX_AGENT_PANE_BACKEND={raw} (unknown, expected tmux|iterm)",
                warn_tag()
            ));
        }
    } else {
        lines.push(format!(
            "{} agent_pane_backend.env: HELLOX_AGENT_PANE_BACKEND is unset",
            ok_tag()
        ));
    }
    ...
    lines.push(format!(
        ...
        if pane_preflight.iterm_available {
            ...
            pane_preflight.iterm_reason.as_str()
        }
    ));
    lines.push(format!(
        ...
        pane_preflight
            .detected_backend
            .as_deref()
            .unwrap_or("none (pane backend will fall back to detached_process)")
    ));

    lines
}

fn inspect_command_prefix_status(lines: &mut Vec<String>, status: &PaneCommandPrefixStatus) {
...
fn command_prefix_resolution_line(label: &str, program: Option<&str>) -> String {
    match program
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .and_then(resolve_command_program_path)
    {
        Some(path) => format!("{} {}: {}", ok_tag(), label, path),
        None => format!(
            "{} {}: unresolved (command may fail at runtime)",
            warn_tag(),
            label
        ),
    }
...
mod tests {
    ...
    fn agent_backend_doctor_lines_report_invalid_pane_command_env() {
        ...
        let text = agent_backend_doctor_lines().join("\n");
