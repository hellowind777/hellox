...
fn frontmatter_list(frontmatter: &BTreeMap<String, String>, key: &str) -> Vec<String> {
    frontmatter
        .get(key)
        .map(|value| {
            value
...
mod tests {
    ...
    use super::{discover_skills, find_skill};
    ...
    fn temp_dir() -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        let root = std::env::temp_dir().join(format!("hellox-config-skills-{suffix}"));
        fs::create_dir_all(&root).expect("create temp dir");
        root
    }

    #[test]
    fn discovers_project_skills_and_keeps_body() {
        let root = temp_dir();
...
Focus on correctness first."#,
        ...
        let detail = find_skill(&skills, "review").expect("find skill");
        assert_eq!(detail.allowed_tools, vec!["Read", "Grep"]);
        assert!(
            detail.body.contains("Review skill body."),
            "{}",
            detail.body
        );
        assert!(
            detail.body.contains("Focus on correctness"),
            "{}",
            detail.body
        );
    }
